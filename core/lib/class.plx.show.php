...
class plxShow {
	...
	/**
	 * Méthode qui affiche le nombre de likes (sous forme de lien ou non selon le mode) d'un article
	 *
	 * @param	f1		format d'affichage si nombre de commentaire = 0 (#nb pour afficher le nombre de commentaire)
	 * @param	f2		format d'affichage si nombre de commentaire = 1 (#nb pour afficher le nombre de commentaire)
	 * @param	f3		format d'affichage si nombre de commentaire > 1 (#nb pour afficher le nombre de commentaire)
	 ...
	 * @author	Stephane F
	 **/
	public function artNbLikes($f1='L_NO_LIKES',$f2='#nb L_LIKES',$f3='#nb L_LIKES') {

		$nb = intval($this->plxMotor->plxRecord_arts->f('nb_likes'));
		$num = intval($this->plxMotor->plxRecord_arts->f('numero'));
		$url = $this->plxMotor->plxRecord_arts->f('url');

		if($nb==0) {
			$txt = str_replace('L_NO_LIKES', L_NO_LIKES, $f1);
			$title = $nb.' '.L_NO_LIKES;
		}
		elseif($nb==1) {
			$txt = str_replace('L_LIKES', L_LIKES, $f2);
			$title = $nb.' '.L_LIKES;
		}
		else {
			$txt = str_replace('L_LIKES', L_LIKES, $f3);
			$title = $nb.' '.L_LIKES;
		}
		$txt = str_replace('#nb',$nb,$txt);
	...
	/**
	 * Méthode qui affiche le nombre de hates (sous forme de lien ou non selon le mode) d'un article
	 *
	 * @param	f1		format d'affichage si nombre de commentaire = 0 (#nb pour afficher le nombre de commentaire)
	 * @param	f2		format d'affichage si nombre de commentaire = 1 (#nb pour afficher le nombre de commentaire)
	 * @param	f3		format d'affichage si nombre de commentaire > 1 (#nb pour afficher le nombre de commentaire)
	 ...
	 * @author	Stephane F
	 **/
	public function artNbHates($f1='L_NO_HATES',$f2='#nb L_HATES',$f3='#nb L_HATES') {

		$nb = intval($this->plxMotor->plxRecord_arts->f('nb_hates'));
		$num = intval($this->plxMotor->plxRecord_arts->f('numero'));
		$url = $this->plxMotor->plxRecord_arts->f('url');

		if($nb==0) {
			$txt = str_replace('L_NO_HATES', L_NO_HATES, $f1);
			$title = $nb.' '.L_NO_HATES;
		}
		elseif($nb==1) {
			$txt = str_replace('L_HATES', L_HATES, $f2);
			$title = $nb.' '.L_HATES;
		}
		else {
			$txt = str_replace('L_HATES', L_HATES, $f3);
			$title = $nb.' '.L_HATES;
		}
		$txt = str_replace('#nb',$nb,$txt);
	...
	/**
	 * Méthode qui affiche le nombre avarange rating d'un article
	 *
	 * @param	f1		format d'affichage si nombre de commentaire  = 0 (#nb pour afficher le nombre de commentaire)
	 * @param	f2		format d'affichage si nombre de commentaire >= 1 (#nb pour afficher le nombre de commentaire)
	 * @param	f3      reserved
	 ...
	 * @author	Stephane F
	 **/
	public function artNbRating($f1='L_NO_RATING',$f2='#nb L_RATING',$f3 = NULL) {
 		$nb = intval($this->plxMotor->plxRecord_arts->f('nb_rate'));
		$art_num = intval($this->plxMotor->plxRecord_arts->f('numero'));
		$url = $this->plxMotor->plxRecord_arts->f('url');

		if($nb==0) {
			$txt = str_replace('L_NO_RATING', L_NO_RATING, $f1);
			$title = $nb.' '.L_NO_RATING;
		}
		elseif($nb==1) {
			$txt = str_replace('L_RATING', L_RATING, $f2);
			$title = $nb.' '.L_RATING;
		}
		else {
			$txt = str_replace('L_RATING', L_RATING, $f3);
			$title = $nb.' '.L_RATING;
		}
		$txt = str_replace('#nb',$nb,$txt);
		...
		if($this->plxMotor->mode == 'article')
			...
			echo '<a href="'.$this->plxMotor->urlRewrite('?article'.$art_num.'/'.$url).'#comments" title="'.$title.'">'.$txt.'</a>';
	...
	public function lastArtList($format='<li><a href="#art_url" title="#art_title">#art_title</a></li>',$max=5,$cat_id='',$ending='', $sort='rsort') {
		...
		if($aFiles = $plxGlob_arts->query($motif,'art',$sort,0,$max,'before')) {
			foreach($aFiles as $v) { # On parcourt tous les fichiers
				...
				$row = str_replace('#art_nblikes',$art['nb_like'],$row);
				$row = str_replace('#art_nbhates',$art['nb_hate'],$row);
