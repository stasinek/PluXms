...
class plxUtils {
	...
	*/
	 ...
	 *
	 * @param	str			chaine de caractères à couper
	 * @param	n_start     start tag
	 * @param   n_end		end tag
	 * @param	replacement	text to put instead
	 * @return	string		chaine de caractères coupée
	 **/
    public static function strGetBetween($str, $needle_start, $needle_end) {
        $pos = strpos($str, $needle_start);
        $start_pos = ($pos === false) ? 0 : $pos + strlen($needle_start);
        $pos = strpos($str, $needle_end, $start_pos);
        $end_pos = ($pos === false) ? strlen($str) : $pos;
        if (($end_pos-$start_pos)==0) return null;
        else return substr($str, $start_pos, $end_pos - $start_pos);
    }
	/**
	 ...
	 * @return	string		chaine de caractères coupée
	 **/
    public static function strReplaceBetween($str, $needle_start, $needle_end, $replacement) {
        $pos = strpos($str, $needle_start);
        $start_pos = ($pos === false) ? 0 : $pos + strlen($needle_start);
        $pos = strpos($str, $needle_end, $start_pos);
        $end_pos = ($pos === false) ? strlen($str) : $pos;
        if (($end_pos-$start_pos)==0) return null;
        else return substr_replace($str, $replacement, $start, $end - $start);
    }
	/**
