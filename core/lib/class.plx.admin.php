...
';
	...
	public function editArticle($content, &$id) {
        ...
        # Likes and hated
		if ($content['likes']) $xml .= "\t".'<likes>'.$content['likes'].'</likes>'."\n";
		else $xml .= "\t".'<likes>0</likes>'."\n";
		if ($content['hates']) $xml .= "\t".'<hates>'.$content['hates'].'</hates>'."\n";
		else $xml .= "\t".'<hates>0</hates>'."\n";
        # Averange rate
	...
	public function editCommentaire($content, &$id) {
		...
		if (isset($com['avatar']))	$comment['avatar'] = $com['avatar'];
            else if (isset($content['avatar'])) $comment['avatar'] = $content['avatar'];
                    else $comment['avatar'] = '1';
		$comment['ip'] = $com['ip'];
		$comment['type'] = $com['type'];
		$comment['mail'] = $content['mail'];
		$comment['site'] = $content['site'];
		$comment['parent'] = $com['parent'];
		if (isset($com['likes'])) $comment['likes'] = $com['likes'];
            else $comment['likes'] = '0';
		if (isset($com['hates'])) $comment['hates'] = $com['hates'];
            else $comment['hates'] = '0';
	...
	/**
	 * Like or hate un commentaire
	 *
	 * @param	id	identifiant du commentaire à traiter (que l'on retourne)
	 * @param   like_minus_hate +1 like to -1 hate
	 * @return	string
	 * @author	Stanislaw Stasiak. et Florent MONTHEL
	 **/
	public function rateCommentaire(&$id,$like_minus_hate) {
		# Génération du nom du fichier
		$oldfilename = PLX_ROOT.$this->aConf['racine_commentaires'].$id.'.xml';
		if(!file_exists($oldfilename)) # Commentaire inexistant
			return plxMsg::Error(L_ERR_UNKNOWN_COMMENT);
		...
		if($like_minus_hate > 1 OR $like_minus_hate < -1) {
			return plxMsg::Error(L_COMMENT_VALIDATE_ERR);
        }		
        // Open XML file, if likes/hates tag exist: increment and replace it's value, if not, put at the end of file, fastest possible & safe
        // Try 10 times to lock file exclisivly if it;s not possible return error & close file without modifications.
        // ^^ This will give chance to rate for many people "same time".
        // TODO: add IP,User ID filtering prevents rating more than once ever + plugin connect likes to Facebook
        // TODO: rate TAG exist only for rating article, but this is universal function, can rate comment in future
		
		$filehandle = fopen($oldfilename, "r+");
		for ($i=0;$i<10;$i++) {
            if (flock($filehandle,LOCK_EX)) 
                break; 
            else { if (i==9) { fclose($filehandle); return plxMsg::Error(L_COMMENT_VALIDATE_ERR); } else sleep(1); }
        }
		$filestat = fstat($handle);
		$fcontent = fread($filehandle,$filestat['size']);
		$tags = array("<hates>","<rating>","<likes>","</hates>","</rating>","</likes>");
		
		$likes = plxUtils::strGetBetween($fcontent,$tags[1-$like_minus_hate],$tags[3-$like_minus_hate]);
		if (isset($likes)) plxUtils::strReplaceBetween($fcontent,$tags[1-$like_minus_hate],$tags[3-$like_minus_hate],$likes+$like_minus_hate);
		else { str_replace($fcontent,"</comment>",$tags[1-$like_minus_hate]."1".$tags[1-$like_minus_hate]."\n</comment>"); }
		
		fwrite($filehandle, $fcontent);
        fclose($filehandle);
	...
	public function likeCommentaire(&$id) {
        return rateCommentaire($id,+1);
    }
	public function hateCommentaire(&$id) {
        return rateCommentaire($id,-1);
    }
		/**
	 * Like or hate un commentaire
	 ...
	 * @author	Stanislaw Stasiak. et Florent MONTHEL
	 **/
	public function rateArticle(&$id,$like_minus_hate,$rate = NULL) {
		# Génération du nom du fichier
		$oldfilename = PLX_ROOT.$this->aConf['racine_articles'].$id.'.xml';
		if(!file_exists($oldfilename)) # Article inexistant
		...
		for ($i=0;$i<10;$i++) {
            if (flock($filehandle,LOCK_EX)) 
                break; 
            else { if (i==9) {fclose($filehandle); return plxMsg::Error(L_ARTICLE_SAVE_ERR); } else sleep(1); }
            }
		$filestat = fstat($handle);
		$fcontent = fread($filehandle,$filestat['size']);
		$tags = array("<hates>","<rating>","<likes>","</hates>","</rating>","</likes>");
	...
	public function likeArticle(&$id) {
        return rateArticle($id,+1);
    }
	public function hateArticle(&$id) {
        return rateArticle($id,-1);
    }
	/**
