...
class plxMotor {
	...
	public function parseArticle($filename) {
		...
		// LIKES,DISLIKES numero (-50 limit comment, email to author to be sent)
 		if(isset($iTags['likes']))
			$art['likes'] = plxUtils::getValue($values[$iTags['likes'][0]]['value'],'0');
		else
			$art['likes'] = '0';
 		if(isset($iTags['hates']))
			$art['hates'] = plxUtils::getValue($values[$iTags['hates'][0]]['value'],'0');
		else
			$art['hates'] = '0';
		// RATING numero
 		if(isset($iTags['rating']))
			$art['rating'] = plxUtils::getValue($values[$iTags['rating'][0]]['value'],'0');
		else
			$art['rating'] = '0';
		...
		$art['nb_likes'] = $this->getNbLikes('/^'.$art['numero'].'.[0-9]{10}.[0-9]+.xml$/');
		$art['nb_hates'] = $this->getNbHates('/^'.$art['numero'].'.[0-9]{10}.[0-9]+.xml$/');
		$art['nb_rating'] = $this->getNbRating('/^'.$art['numero'].'.[0-9]{10}.[0-9]+.xml$/');
	...
	/**
	 * Méthode qui retourne le nombre de likes respectants le motif $motif et le paramètre $publi
	 *
	 * @param	motif	motif de recherche des likes
	 * @param	publi	before, after ou all => on récupère tous les fichiers (date) ?
	 * @return	integer
	 * @author	Stanislaw Stasiak
	 **/
	public function getNbLikes($motif,$publi='before') {

		return 0;
	}
	/**
	 * Méthode qui retourne le nombre de hates respectants le motif $motif et le paramètre $publi
	 *
	 * @param	motif	motif de recherche des hates
	 * @param	publi	before, after ou all => on récupère tous les fichiers (date) ?
	 * @return	integer
	 * @author	Stanislaw Stasiak
	 **/
	public function getNbHates($motif,$publi='before') {

		return 0;
	}
	/**
	 * Méthode qui retourne le nombre de rating respectants le motif $motif et le paramètre $publi
	 *
	 * @param	motif	motif de recherche des hates
	 * @param	publi	before, after ou all => on récupère tous les fichiers (date) ?
	 * @return	integer
	 * @author	Stanislaw Stasiak
	 **/
	public function getNbRating($motif,$publi='before') {
	...
	public function parseCommentaire($filename) {
		...
		// AVATAR numero.css .comment-av1,.comment-av2 etc. >>
 		if(isset($iTags['avatar']))
			$com['avatar'] = plxUtils::getValue($values[ $iTags['avatar'][0]]['value'],'1');
		else
			$com['avatar'] = '1';
		// AVATAR numero
		if(isset($iTags['type']))
			...
			$com['type'] = 'normal';
		// LIKES,DISLIKES numero (-5 limit comment to be seen)
 		if(isset($iTags['likes']))
			$com['likes'] = plxUtils::getValue($values[ $iTags['likes'][0]]['value'],'0');
		else
			$com['likes'] = '0';
 		if(isset($iTags['hates']))
			$com['hates'] = plxUtils::getValue($values[ $iTags['hates'][0]]['value'],'0');
		else
			$com['hates'] = '0';
		// DISLIKES numero
	...
	public function newCommentaire($artId,$content) {
		...
		if($this->aConf['capcha'] == 0 OR $_SESSION['capcha'] == sha1($content['rep'])) {
			if(!empty($content['name']) AND !empty($content['content'])) { # Les champs obligatoires sont remplis
				...
				if (isset($content['avatar']) ? !empty($content['avatar']) : false)
					$comment['avatar'] = plxUtils::strCheck(trim($content['avatar']));
				else 
					$comment['avatar'] = '1';
				$comment['likes'] = 0;
				$comment['hates'] = 0;
				...
				if(isset($content['parent']) ? !empty($content['parent']) : false) {
	...
	public function addCommentaire($content) {
		...
		$xml .= "\t<avatar>".$content['avatar']."</avatar>\n";
